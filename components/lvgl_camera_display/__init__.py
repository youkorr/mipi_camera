...
DEPENDENCIES = ["lvgl", "mipi_camera"]
AUTO_LOAD = ["mipi_camera"]
...
# Import du namespace mipi_camera avec le BON nom de classe
mipi_camera_ns = cg.esphome_ns.namespace("mipi_camera")
MIPICameraComponent = mipi_camera_ns.class_("MIPICameraComponent")

CONFIG_SCHEMA = cv.Schema({
    cv.GenerateID(): cv.declare_id(LVGLCameraDisplay),
    cv.Required(CONF_CAMERA_ID): cv.use_id(MIPICameraComponent),  # ← Correction ici
